...
function add_3d_monomer(chain)
    ...
    if length(chain)==1
        push!(chain, [rand(1:20), dict_direction[rand(1:6)]])
    else
...
function build_3d_protein(n)
    ...
    protein = [[rand(1:20), [0, 0, 0]]]
    for _ in 1:n-1
        protein = add_3d_monomer(protein)
    end
    return protein
end

function plot_3d_protein(protein, title)
    ...
    z_coords = [point[2][3] for point in protein]
